...
namespace Catalogue.API.Web.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        public ProductController(IProductCreate createNewItem, IProductRemove removeItem, IProductUpdate updateItem, IGetAllProducts getAllProducts,
            ...
            IAddProductToCart addProductToCart, IPublishNewMessage publishNewMessage)
        {
        ...
        public async Task<IActionResult> UpdateProductAsync(Guid id, UpdateProductModel productModel)
        {
            if (ModelState.IsValid)
            {
                var productForUpdate = await updateItem.UpdateItemAsync(id, productModel, User);
                publishNewMessage.PublishUpdatedProductData(productForUpdate);
        ...
        {
            var removedItem = await removeItem.RemoveProductByIdAsync(id);
            publishNewMessage.PublishRemovedProductEvent(removedItem);
        ...
        {
            var addedProductModel = await addProductToCart.AddProductToCartAsync(productId, quantity, User);
            publishNewMessage.PublishAddProductToCartEvent(addedProductModel);
